...
export const backend:Backend=desktop?{mode:'desktop',updates:()=>invoke('update_status'),updateAction:action=>invoke('update_action',{action}),snapshot:()=>invoke('snapshot'),activity:()=>invoke('activity'),savePolicy:policy=>invoke('save_policy',{policy}),recreateWorker:policy=>invoke('recreate_worker',{policy}),action:action=>invoke('worker_action',{action}),enroll:(url,code)=>invoke('enroll',{url,code}),fleet:()=>invoke('fleet')}:{
