...
export const backend:Backend=desktop?{mode:'desktop',updates:()=>invoke('update_status'),updateAction:action=>invoke('update_action',{action}),snapshot:()=>invoke('snapshot'),activity:()=>invoke('activity'),savePolicy:policy=>invoke('save_policy',{policy}),recreateWorker:policy=>invoke('recreate_worker',{policy}),previewStorage:(selections,options)=>invoke('preview_storage',{selections,...(options?{options}:{})}),setStorageRecovery:enabled=>invoke('set_storage_recovery',{enabled}),retryStorageMaintenance:()=>invoke('retry_storage_maintenance'),applyStorage:plan=>invoke('apply_storage',{plan}),chooseStorageDirectory:()=>invoke('choose_storage_directory'),action:action=>invoke('worker_action',{action}),enroll:(url,code)=>invoke('enroll',{url,code}),fleet:()=>invoke('fleet')}:{
