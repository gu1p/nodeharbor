...
export function SharingRules({policy,host,onChange,onSave,busy}:{policy:Policy;host:Resources;onChange:(policy:Policy)=>void;onSave:()=>void;busy:boolean}) {
 ...
 <div className="section-heading"><div><h1>Sharing rules</h1><p>Choose how much to share, and when your computer is available.</p></div><button className="primary" disabled={busy} type="submit">{busy?'Saving…':'Save sharing rules'}</button></div>
 <section className="panel"><h2>Your resource budget</h2><p>This allowance includes the Linux worker and its running containers.</p><div className="resource-inputs">
