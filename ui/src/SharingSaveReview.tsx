...
export function SharingSaveReview({review,inventory,busy,error,onCancel,onSave,saveButton,onPause,onStop}:{review:SharingReview|null;inventory?:StorageInventory;busy:boolean;error:string;onCancel:()=>void;onSave:()=>void;saveButton:RefObject<HTMLButtonElement|null>;onPause?:()=>void;onStop?:()=>void}) {
 ...
 <AlertDialog.Description>{review?.plan.requiresRestart?(review.policy.enabled?'Saving these changes drains running work and restarts the worker.':'These storage changes will be applied when you prepare the worker or start sharing.'):'These settings and storage locations will be used when you prepare the worker.'}</AlertDialog.Description>
 {review&&<><p>{review.policy.resources.cpus} CPU cores · {review.policy.resources.memoryMib/1024} GiB RAM · {review.plan.totalGib} GiB total VM storage</p>
 ...
 {review.plan.layout?<p>{review.plan.layout.systemGib} GiB system + {review.plan.totalGib-review.plan.layout.systemGib} GiB workload storage, before filesystem overhead. VM directory: <code>{review.plan.layout.runtimeDirectory}</code>.</p>:inventory?.systemDisk&&<p>Separate system disk: {inventory.systemDisk.allocationGib} GiB in <code>{inventory.systemDisk.directory}</code>.</p>}
