...
class UpdateChannel(unittest.TestCase):
 def test_every_installer_format_gets_a_signed_target_without_public_release_clutter(self):
  with tempfile.TemporaryDirectory() as directory:
   ...
   self.assertEqual(set(manifest['platforms']),{'darwin-aarch64-app','darwin-x86_64-app','linux-aarch64-deb','linux-x86_64-deb','linux-aarch64-appimage','linux-x86_64-appimage','windows-x86_64-nsis','android-aarch64-apk'})
   self.assertTrue(manifest['platforms']['android-aarch64-apk']['url'].endswith('.apk'))
