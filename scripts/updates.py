...
def build(folder: Path, output: Path, release_version: str, commit: str, key: Path):
    ...
    for target in manifests:
        os_name = {'macos': 'darwin', 'linux': 'linux', 'windows': 'windows'}[target['os']]
        arch = {'arm64': 'aarch64', 'amd64': 'x86_64'}[target['arch']]
        for asset in target['assets']:
            name = asset['name']
            extension = next((suffix for suffix in ['.app.tar.gz', '.AppImage', '.deb', '.exe'] if name.endswith(suffix)), None)
            if extension is None:
                continue
            installer = {'.app.tar.gz': 'app', '.AppImage': 'appimage', '.deb': 'deb', '.exe': 'nsis'}[extension]
